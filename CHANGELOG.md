...
### Added
- **Quotation-mark unifier** (`crisp-docx-core` module `quote_style` + `crisp-docx normalize-quotes` CLI): rewrites every quotation mark across `document.xml` / `footnotes.xml` / `endnotes.xml` to one national style — German `„…“`, English `“…”`, French `« … »` (narrow no-break inner spacing), Swiss `«…»` (no spacing), and inward German guillemets `»…«` / `›…‹` (the reverse of French — `--style german-guillemets`, aliases `inverse` / `reversed`). Open vs. close is inferred from context (not the ambiguous glyph) and threaded across runs; apostrophes (`l'homme`, `Cusanus'`) are preserved; `--skip-singles` limits it to double quotes. Converting away from French strips guillemet inner spacing.
