...
each defaults to a repo-relative path below (or this directory) and can be
overridden via an env var. CI cannot run the parity harness without these
fixtures present; CI runs the unit/integration suite only.

| Fixture | Path (env var override) | Purpose |
|---|---|---|
| `vielfalt_cs15` | `$CRISP_DOCX_PARITY_VIELFALT` (default: `tests/parity_fixtures/vielfalt_cs15.docx`) | Large real document with 46 footnotes; pandoc-built via the Python pipeline. |
